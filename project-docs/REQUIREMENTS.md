...
- **RS485 hub: CDEBYTE E810-R14** (1→4 irányú, opto-izolált RS485 repeater/hub) – a telepítés fizikai adottságai miatt a szenzorok nem köthetők egy szál kábelre daisy-chain-nel, csak csillag-topológiában; ez az eszköz oldja meg a csillag-kábelezést anélkül, hogy protokoll-szinten bármit is módosítana. Hivatalos user manual (letöltve, ellenőrzött forrás): [`docs/hardver/e810-r1x-rs485-hub-user-manual.pdf`](docs/hardver/e810-r1x-rs485-hub-user-manual.pdf).
  ...
  - Baud tartomány `300–230400` (egyéni baud is támogatott) – a szenzor `1200–115200` tartományát bőven lefedi. A busz össz-eszközszám korlátja `32` – ez az általunk tervezett 8 slot-nak bőven elég tartalékot ad.
...
- **Konfigurációs UX**: nem egyszerű be/kikapcsoló kapcsoló, hanem **Hozzáadás/Törlés** – ez **kizárólag az ESP saját webes felületén**, natív ESPHome-eszközökkel (nincs HA-függés, nincs új custom komponens, sima YAML + lambda):
  - a "táblázatos, egymás alá rendezett" elrendezést a `web_server: sorting_groups` adja – slotonként csoportosított sorok ("Slot N" fejléc alatt cím/Display Name/törlés), nem literál HTML `<table>`, de gyakorlatilag ezt a hatást adja,
  - "Hozzáadás" a soron következő szabad slotot nyitja meg beviteli mezőkkel – a még nem betanított slotok futásidőben `set_internal(true)`-val rejtettek (ez az `EntityBase` runtime setter-e, nem az `internal:` fordításidejű YAML-kulcs), "Hozzáadás"-ra a következő slot `set_internal(false)`-ra vált. **Ellenőrizendő a valós eszközön**: nem biztos, hogy a web_server SSE-alapú élő frissítése oldalújratöltés nélkül is felveszi az újonnan megjelenő entitást.
  - "Törlés" natív JS `confirm()` nélkül, **kétlépéses megerősítéssel**: első kattintásra a gomb "Biztos? Töröl"-re vált, egy rövid (pl. `10s`) `script`/`interval` timeout után automatikusan visszaáll, ha nem jön a második kattintás – csak a második kattintásra törli ténylegesen a slot adatait (cím, Display Name, kalibráció) és áll vissza "nincs betanítva" alapállapotba,
