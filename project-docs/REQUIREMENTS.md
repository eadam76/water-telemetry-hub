...
- **Modbus regisztertérkép** – ⚠️ eredetileg **nem hivatalos forrásból, közösségi/kompatibilitási dokumentációból (TapHome kompatibilitási oldal, Home Assistant közösségi fórum – ugyanerre a `QDW90A-3`, 0–10 bar variánsra vonatkozó, egymástól független bejegyzések) összeállítva** – az alábbi pontok közül a **slave cím és a nyomásérték regisztere már megerősítve valós hardveren** (ld. lent), a skálázás/range regiszterek még nem:
  ...
  - **Nyomásérték: `4`-es regiszter – ✅ regiszter megerősítve, élőben reagál valós nyomásra**: nyugalomban `0` (várt érték terheletlen/légköri állapotban), majd a nyomáscsatlakozóba szájjal belefújva (a tényleges mérési úton, nem mechanikai membrán-nyomogatással) folyamatosan, monoton nőtt: `0 → 3 → 4 → 5` nyers érték (mindegyik CRC-vel ellenőrizve, valódi válasz). A **`/100` skálázás ezzel elég meggyőzően alátámasztott** (`0.00–0.05 bar` egy finom-közepes szájjal fújt nyomástól – emberi fújásnyomás jellemzően kb. `0.01–0.05 bar` tartományba esik –, `0–10 bar` méréshatáron ez pontosan a várt kis kitérés) – de továbbra sem kalibrált-pontos (nincs ismert, mért referencianyomáshoz hasonlítva), csak jól egyező nagyságrend.
