...
- **Cím módosítása regisztrált eszközön**: a terv szerint ez **ténylegesen újraprogramozza a fizikai eszközt is** (a dokumentált kétlépéses Modbus írás+mentés szekvenciával, ld. `docs/hardver/qdw90a-modbus-referencia.md`), nem csak a mi rekordunkat írja át – utána a rekord frissül, hogy kövesse az új címet. **Ez a rész még nem implementált** (ld. lent) – jelenleg csak a rekordot írja át, a valódi Modbus híján ez egyelőre nem is tehető meg.
...
- **Státusz**: implementálva, 2026-08-13 (mock busz-scannel, a jelenlegi esp32dev teszt-panelen kipróbálható) – `packages/pressure_sensor.yaml` (slotonként cím+név, `try_register` szkript), `water-collector.yaml` (8 slot, mock "Scan Bus" gomb + CSV-bemenet, megosztott "Add" trigger), `web/dashboard.js` (JOIN-alapú, három-állapotú táblázat-renderelés). Hátralévő rész, a Waveshare panelre vár: **valódi UART/Modbus busz-scan** (a mock gomb/CSV-mező helyett), és **cím-módosításkor a fizikai eszköz tényleges újraprogramozása** (fent) – ezek nélkül a tábla ma csak a felhasználó által begépelt mock-címekkel "lát" eszközöket.

### Jövőbeli, hardverfüggő ötletek (nincs implementálva – valós RS485/Modbus hardver kell a kipróbálásukhoz)

- **Busz-szkennelős betanítás**: betanításkor a busz automatikus végigpásztázása (`1–247` cím, rövid timeout-tal), hogy listázza, mely Modbus-címek elérhetők, de még nincsenek felvéve a mi listánkba – kényelmesebb lenne, mint kézzel beírni a címet. A cím **írása/mentése/olvasása már pontosan ismert és dokumentált** (ld. `docs/hardver/qdw90a-modbus-referencia.md` – gyártói hivatalos Modbus-doksi alapján, 2026-08-13), csak maga a "pásztázás" (sok cím sorban végigpróbálása) igényel valós hardvert a kipróbáláshoz – ez **betanítás-idejű, nem folyamatos** funkció lenne.
- **Cím-váltás a már bekötött eszközön (varázsló)**: a pontos, hardveren megerősített eljárás megvan (ld. `qdw90a-modbus-referencia.md` "Betanítás / cím módosítása") – **fontos részlet**: ez **nem egy Modbus-művelet**, a cím-írás válasza még a régi címről jön, a mentést (`H:15=0`) már az **új** címre kell küldeni. Ezt a kétlépéses logikát a firmware-nek/UI-nak kell kezelnie.
