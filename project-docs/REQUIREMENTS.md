...
- **Modbus regisztertérkép** – ⚠️ eredetileg **nem hivatalos forrásból, közösségi/kompatibilitási dokumentációból (TapHome kompatibilitási oldal, Home Assistant közösségi fórum – ugyanerre a `QDW90A-3`, 0–10 bar variánsra vonatkozó, egymástól független bejegyzések) összeállítva** – az alábbi pontok közül a **slave cím és a nyomásérték regisztere már megerősítve valós hardveren** (ld. lent), a skálázás/range regiszterek még nem:
  ...
  - **Nyomásérték: `4`-es regiszter – ✅ regiszter megerősítve, élőben reagál valós nyomásra**: nyugalomban `0` (várt érték terheletlen/légköri állapotban), majd kézzel a membránra nyomva folyamatosan, monoton nőtt: `0 → 3 → 4 → 5` nyers érték (mindegyik CRC-vel ellenőrizve, valódi válasz). A **`/100` skálázás ezzel nagyságrendileg alátámasztott** (`0.00–0.05 bar` egy finom ujjnyomástól, `0–10 bar` méréshatáron ez plauzibilis kis kitérés) – **de nem kalibrált-pontos** (nincs ismert referencianyomáshoz mérve), csak a nagyságrend stimmel.
