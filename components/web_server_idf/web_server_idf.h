...
class AsyncEventSourceResponse {
 ...
 protected:
  ...
  // Local fix (see water-telemetry-hub.yaml / REQUIREMENTS.md for the writeup): set once we've asked
  // httpd to close a stuck connection, so we stop touching event_buffer_/deferred_queue_ and stop
  // re-triggering the close while the real close is still in flight. Deliberately NOT the same
  // thing as fd_==0 - see process_buffer_() in the .cpp for why conflating the two was the bug.
