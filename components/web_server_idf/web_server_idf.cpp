...
// ESPHOME_VERSION_CODE / VERSION_CODE, for the one place below that has to
// build against both an ESPHome that has ComponentIterator::try_advance()
...
void AsyncWebServer::begin() {
  ...
  // water-telemetry-hub.yaml - that has to go up too, or this can't actually get the sockets
...
void AsyncEventSourceResponse::process_buffer_() {
  // Local fix: once we've asked httpd to close a stuck connection, do nothing further until
  // httpd actually retires the session (destroy() below, via the real close_fn path) sets
  // fd_ to 0 and the main-loop sweep deletes us. See the closing_requested_ branch below for
  // why this guard has to exist.
  if (this->closing_requested_) {
    return;
  }
  ...
  if (bytes_sent == HTTPD_SOCK_ERR_TIMEOUT) {
    ...
    if (this->consecutive_send_failures_ >= MAX_CONSECUTIVE_SEND_FAILURES) {
      ...
      //
      // UPSTREAM BUG FIX (not yet fixed in esphome/esphome as of 2026.7.3 / current main -
      // see the writeup in water-telemetry-hub.yaml): the original code here did
      // `this->fd_.store(0)` directly, which makes the next AsyncEventSource::loop() tick
      // `delete` this object immediately - but httpd's own session (req->sess_ctx) still
      // points at it, and httpd's free_ctx callback (destroy(), below) hasn't run yet.
      // When httpd later actually retires this socket for real (its own idle/LRU/error
      // handling) it invokes destroy() on that now-dangling pointer - a genuine
      // use-after-free that corrupts the heap. On real hardware this reproduced as a TLSF
      // `block_merge_prev` assertion/heap-corruption crash, detected later and elsewhere
      // (inside lwIP's own housekeeping), which is why it looked unrelated to EventSource
      // at first glance.
      //
      // Fix: don't touch fd_ or delete anything ourselves here. Just ask httpd to close the
      // session through its own API. That runs the same close_fn (safe_close_with_shutdown)
      // and free_ctx (destroy()) path as a normal client-initiated disconnect, which is the
      // only place fd_ is safely allowed to become 0. closing_requested_ stops us from
      // touching the buffers or re-entering this branch while that's in flight.
...
void AsyncEventSourceResponse::loop() {
  ...
  if (!this->entities_iterator_.completed()) {
    // ComponentIterator::advance() is deprecated upstream and is removed
    // in ESPHome 2027.3.0; it is now just a wrapper for try_advance(1),
    // so this is the same call by its new name and nothing about the
    // one-entity-per-loop pacing changes.
    //
    // Version-guarded because this file is a fork that has to keep
    // building against whatever ESPHome it is compiled with, and
    // try_advance() does not exist in older releases (confirmed absent in
    // 2026.6.5, present in 2026.8.1). The bound is deliberately the
    // version it is KNOWN to exist in rather than the one that probably
    // introduced it: guessing too low breaks the build, guessing too high
    // only keeps a deprecated call that still works.
#if defined(ESPHOME_VERSION_CODE) && ESPHOME_VERSION_CODE >= VERSION_CODE(2026, 8, 0)
...
bool AsyncEventSourceResponse::try_send_nodefer(const char *message, size_t message_len, const char *event, uint32_t id,
                                                uint32_t reconnect) {
  // closing_requested_: see process_buffer_() - a real close is already in flight via httpd,
  // don't grow event_buffer_ with more data that will never get sent.
  if (this->fd_.load() == 0 || this->closing_requested_) {
    return false;
  }
